...
// Param order matches the v4 Infinity CL Initialize ABI:
// (id, currency0, currency1, hooks, fee, parameters, sqrtPriceX96, tick).
// tickSpacing is decoded by the handler from bytes [27,30) of `parameters`.
const INITIALIZE_EVENT = new Initialize(
  ...
  [
    ...
    new ethereum.EventParam(
      'parameters',
      ethereum.Value.fromFixedBytes(buildParametersBytes32(parseInt(INITIALIZE_FIXTURE.tickSpacing) as i32)),
    ),
    new ethereum.EventParam(
      'sqrtPriceX96',
...
describe('handleInitialize edge cases', () => {
  beforeEach(() => {
    clearStore()
  })

  test('poolsToSkip: pool listed in skip set is fully ignored (no Pool, no PoolManager, no Bundle)', () => {
    const skipConfig: SubgraphConfig = {
      poolManagerAddress: TEST_CONFIG.poolManagerAddress,
      stablecoinWrappedNativePoolId: TEST_CONFIG.stablecoinWrappedNativePoolId,
      stablecoinIsToken0: TEST_CONFIG.stablecoinIsToken0,
      wrappedNativeAddress: TEST_CONFIG.wrappedNativeAddress,
      minimumNativeLocked: TEST_CONFIG.minimumNativeLocked,
      stablecoinAddresses: TEST_CONFIG.stablecoinAddresses,
      whitelistTokens: TEST_CONFIG.whitelistTokens,
      tokenOverrides: TEST_CONFIG.tokenOverrides,
      poolsToSkip: [USDC_WETH_POOL_ID],
      poolMappings: TEST_CONFIG.poolMappings,
      nativeTokenDetails: TEST_CONFIG.nativeTokenDetails,
    }

    handleInitializeHelper(INITIALIZE_EVENT, skipConfig)

    assert.entityCount('Pool', 0)
    assert.entityCount('PoolManager', 0)
    assert.entityCount('Token', 0)
    assert.entityCount('Bundle', 0)
  })

  test('token decimals unresolvable: handler bails before saving Pool/PoolManager', () => {
    // Build an Initialize event with token addresses that have no usable
    // metadata. We must EXPLICITLY mock decimals() as reverting — matchstick
    // errors on unmocked calls rather than treating them as reverted.
    // fetchTokenDecimals sees the revert → returns null → handler logs and
    // returns before .save() is called on Pool/PoolManager.
    const freshPoolId = '0xdeadbeef00000000000000000000000000000000000000000000000000000001'
    const unknownToken0 = Address.fromString('0x1111111111111111111111111111111111111111')
    const unknownToken1 = Address.fromString('0x2222222222222222222222222222222222222222')

    createMockedFunction(unknownToken0, 'decimals', 'decimals():(uint32)').reverts()
    createMockedFunction(unknownToken1, 'decimals', 'decimals():(uint32)').reverts()

    const event = new Initialize(
      MOCK_EVENT.address,
      MOCK_EVENT.logIndex,
      MOCK_EVENT.transactionLogIndex,
      MOCK_EVENT.logType,
      ...
      MOCK_EVENT.transaction,
      [
        new ethereum.EventParam('id', ethereum.Value.fromFixedBytes(Bytes.fromHexString(freshPoolId) as Bytes)),
        new ethereum.EventParam('currency0', ethereum.Value.fromAddress(unknownToken0)),
        new ethereum.EventParam('currency1', ethereum.Value.fromAddress(unknownToken1)),
        ...
        new ethereum.EventParam('parameters', ethereum.Value.fromFixedBytes(buildParametersBytes32(10))),
    ...
    // Pool was never created — handler returned after decimals lookup failed.
    assert.entityCount('Pool', 0)
    // PoolManager was instantiated in memory but .save() was never reached.
    assert.entityCount('PoolManager', 0)
  })
})
