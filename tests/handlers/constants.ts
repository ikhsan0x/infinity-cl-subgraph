...
// Pack tickSpacing into the v4 Infinity CL `parameters` bytes32 the same way
// the chain emits it: big-endian uint24 at bytes [27, 30). The handler does
// slice(27,30) → reverse() → BigInt.fromByteArray (which reads LE), so the
// on-chain layout that round-trips is MSB-first at byte 27.
// Bytes [30, 32) become hooksRegistration.
export const buildParametersBytes32 = (tickSpacingI32: i32): Bytes => {
  const buf = new Uint8Array(32)
  buf[27] = ((tickSpacingI32 >> 16) & 0xff) as u8
  buf[28] = ((tickSpacingI32 >> 8) & 0xff) as u8
  buf[29] = (tickSpacingI32 & 0xff) as u8
...
export const invokePoolCreatedWithMockedEthCalls = (
  ...
  const parametersValue = buildParametersBytes32(parseInt(tickSpacing) as i32)

  // Param order must match the Initialize event ABI emitted by PoolManager:
  ...
  const parameters = [
    ...
    new ethereum.EventParam('parameters', ethereum.Value.fromFixedBytes(parametersValue)),
...
export const createAndStoreTestPool = (poolFixture: PoolFixture): Pool => {
  ...
  pool.parameters = buildParametersBytes32(tickSpacing)
